...
const reader = require('./read_images');
...
const LABEL_MULTIPLIER = [224,224,224,224,224,224,224,224,224, 1, 1, 1, 1];
//const PENALTY = 100;
/**
...
function customLossFunction(yTrue, yPred) {
  return tf.tidy(() => {
    ...
    // from shape and bounding-box predictions.
    const loss = tf.metrics.meanSquaredError(yTrue.mul(LABEL_MULTIPLIER), yPred);
    /*const [yTc, yTd,yTe] = tf.split(yTrue, [1, 4, 0], 1);
    const [yPc, yPd,yPe] = tf.split(yPred, [1, 4, 0], 1);
    const shape = yTrue.shape;
   
    console.log('Loss check  act :'+yTrue.shape+'  pred ::'+yPred.shape+' len : '+shape[0]+' :: '+yTc+' ::'+yTd);
    const mloss =  tf.metrics.meanAbsoluteError( yTc, yPc);
    const loss = tf.add( tf.metrics.meanSquaredError(yTd, yPd) , mloss);
    */
...
function buildNewHead(inputShape) {
  ...
  newHead.add(tf.layers.dense({units: 1200, activation: 'relu'}));
  // Five output units:
  ...
  newHead.add(tf.layers.dense({units: 13}));
  return newHead;
}
...
(async function main() {
  ...
  const imageDataCanvas = canvas.createCanvas(CANVAS_SIZE, CANVAS_SIZE);
  const synth =      new reader.ObjectDetectionImageReader(imageDataCanvas, tf);
//  const synth =      new synthesizer.ObjectDetectionImageSynthesizer(imageDataCanvas, tf);
 
  const {images, targets} =      await synth.generateBatch(imageDataCanvas);
